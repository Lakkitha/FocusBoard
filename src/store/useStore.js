...
const DEFAULT_STATE = {
  ...
  // Time sessions  [{ id, type:'course'|'project', refId, date, minutes, note }]
  sessions: [],
...
export const useStore = create((set, get) => ({
  ...
  deleteCourse: (id) => {
    set((s) => ({
      ...
      sessions: s.sessions.filter((s2) => !(s2.type === 'course' && s2.refId === id)),
    }))
    get().persist()
  },
  ...
  deleteProject: (id) => {
    set((s) => ({
      ...
      sessions: s.sessions.filter((s2) => !(s2.type === 'project' && s2.refId === id)),
    }))
    get().persist()
  },
