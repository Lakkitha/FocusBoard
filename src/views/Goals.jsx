...
import { CheckSquare, Plus, Trash2, CheckCircle2, Circle, ChevronLeft, ChevronRight, RotateCcw } from 'lucide-react'
...
export default function Goals() {
  ...
  const weekGoals = useMemo(
    () => goals.filter((g) => g.weekKey === activeWeek),
    [goals, activeWeek]
  )
  ...
  const handleCarryOver = () => {
    ...
    const prevGoals = goals.filter((g) => g.weekKey === prevWeek && !g.done)
    prevGoals.forEach((g) => {
  ...
  const prevWeek = addDays(activeWeek, -7)
  const prevIncomplete = goals.filter((g) => g.weekKey === prevWeek && !g.done).length
    ...
    <div className="p-6 space-y-6 max-w-2xl mx-auto">
      ...
      <div className="flex items-center justify-between card py-3">
        ...
        <div className="text-center">
          <p className="text-sm font-medium text-surface-50">{weekLabel(activeWeek)}</p>
          {isCurrentWeek && (
            <span className="text-[11px] font-semibold text-brand-green">Current Week</span>
          )}
        </div>
      ...
      {total > 0 && (
        <div className="card space-y-3">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-bold text-surface-50">{done}/{total} completed</span>
          </div>
      ...
      {isCurrentWeek && (
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-xs ${total >= 5 ? 'text-red-400' : 'text-surface-400'}`}>
              {total}/5 goals
            </span>
          </div>
          <div className="flex gap-2">
            ...
            <button
              ...
              className={`btn-primary flex-shrink-0 flex items-center gap-1.5 ${
                total >= 5 || !input.trim() ? 'opacity-40 cursor-not-allowed' : ''
              }`}
            >
      ...
      {isCurrentWeek && prevIncomplete > 0 && (
        ...
        >
          ...
          Carry over {prevIncomplete} incomplete goal{prevIncomplete > 1 ? 's' : ''} from last week
        </button>
      )}
      ...
      {weekGoals.length === 0 ? (
        <div className="card text-center py-12">
          ...
          {isCurrentWeek && (
            <p className="text-surface-400 text-sm mt-1">Add up to 5 goals to stay focused.</p>
          )}
        </div>
      ) : (
        <div className="space-y-2">
            ...
            >
              ...
              {editId === g.id ? (
                <div className="flex-1 flex gap-2">
                  ...
                  />
                  <button onClick={() => handleSaveEdit(g.id)} className="btn-primary px-3 py-1 text-xs">
                    Save
                  </button>
                </div>
              ) : (
                <span
                  onClick={() => { setEditId(g.id); setEditText(g.text) }}
                  className={`flex-1 text-sm cursor-pointer ${
                    g.done ? 'line-through text-surface-400' : 'text-surface-100 hover:text-surface-50'
                  }`}
                >
                  {g.text}
                </span>
